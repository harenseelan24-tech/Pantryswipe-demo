...
// Per-step large icon + hint text shown above the step title
const STEP_META = [
  ...
  { emoji: "🥗", hint: "Any dietary needs to respect?" },
  { emoji: "⏰", hint: "When are guests arriving?" },
  { emoji: "✨", hint: "Last personal touches (optional)" },
];

// Accent colours for each plan section's header banner
...
// Per-category colours for the budget bar chart
...
export default function PartyPlannerScreen() {
  ...
  function validateStep(): boolean {
    ...
    switch (wizardStep) {
      ...
      case 4:
        ...
        break;
      case 6:
        if (!form.arrivalTime.trim()) { setStepError("Please select a guest arrival time."); return false; }
        break;
    }
    return true;
  }
  ...
  // ────────────────────────────────────────────────────────────────────────
  // RENDER: segmented step progress dots
  // ────────────────────────────────────────────────────────────────────────
  function renderProgress() {
    return (
      <View style={[s.progressWrap, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
        {Array.from({ length: totalSteps }, (_, i) => {
          ...
          return (
            <View key={step} style={[
              s.progressDot,
              done  && { backgroundColor: colors.primary, width: 10, height: 10 },
              active && { backgroundColor: colors.primary, width: 24, height: 10, borderRadius: 5 },
              !done && !active && { backgroundColor: colors.muted, width: 10, height: 10 },
            ]} />
          );
        })}
      </View>
  ...
  // ────────────────────────────────────────────────────────────────────────
  // RENDER: step header with large emoji
  // ────────────────────────────────────────────────────────────────────────
        ...
        <View style={s.quickAddRow}>
          {GUEST_QUICK_ADD.map((n) => (
            <TouchableOpacity
              ...
              onPress={() => {
                const next = Math.min(form.guestCount + n, 500);
                setForm((f) => ({ ...f, guestCount: next }));
                Haptics.selectionAsync();
              }}
            >
        ...
        <View style={s.presetRow}>
          {BUDGET_PRESETS.map((amt) => {
            ...
            return (
              ...
              >
                <Text style={[s.presetText, { color: active ? colors.primary : colors.foreground }]}>
                  ${amt}
                </Text>
              </TouchableOpacity>
            );
          ...
          {SERVING_STYLES.map((style) => {
            ...
            return (
              ...
              >
                ...
                <Text style={[s.servingDesc, { color: active ? "rgba(255,255,255,0.8)" : colors.mutedForeground }]}>
                  {style.desc}
                </Text>
              </TouchableOpacity>
            );
    ...
    return (
      <View>
        ...
        <Text style={[s.stepHint, { color: colors.mutedForeground }]}>
          Optional — kids attending, favourite dishes, cheese platter, spice level…
        </Text>
        <TextInput
  ...
  // ────────────────────────────────────────────────────────────────────────
  // RENDER: plan cards with coloured section header banners
  // ────────────────────────────────────────────────────────────────────────
          ...
          return (
            <View key={i} style={s.timelineRow}>
              ...
              <View style={s.timelineContent}>
                {time ? (
                  <Text style={[s.timelineTime, { color: SECTION_ACCENT["PREPARATION TIMELINE"] }]}>{time}</Text>
                ) : null}
                <Text style={[s.timelineAction, { color: isLast ? colors.secondary : colors.foreground, fontWeight: isLast ? "700" : "400" }]}>
                  {action}
                </Text>
              </View>
            </View>
...
const s = StyleSheet.create({
  ...
  // Step header
  stepLabelWrap: { marginBottom: 20, gap: 4 },
  ...
  stepHint: { fontSize: 14, lineHeight: 20, marginBottom: 12, marginTop: -12 },
  ...
  // Budget
  budgetRow: { flexDirection: "row", alignItems: "center", borderRadius: 16, borderWidth: 1.5, paddingHorizontal: 18, paddingVertical: 16, gap: 4 },
  ...
  backBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 18, paddingVertical: 16, borderRadius: 100, borderWidth: 1.5, minHeight: 54 },
  ...
  // Plan cards — coloured banner header
